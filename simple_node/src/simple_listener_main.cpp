...
int main(int argc, char * argv[])
{
  ...
  if (argc < 3) {
    RCLCPP_ERROR(rclcpp::get_logger("main"), "Usage: simple_listener <node_name> <path_to_yaml>");
    return 1;
  }
  ...
  if (node_config["type"].as<std::string>() != "listener") {
    RCLCPP_ERROR(rclcpp::get_logger("main"), "Node '%s' is not of type 'listener'.", node_name.c_str());
    return 1;
  }
